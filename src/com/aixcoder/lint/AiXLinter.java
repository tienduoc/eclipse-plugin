...
//		String url = Preference.getEndpoint() + "lint";
		...
		for (int i = 0; i < jo.size(); i++) {
			...
			for (int k = 0; k < r.size();k++) {
				int start = r.get(k).getAsJsonObject().get("start").getAsInt();
				int length = r.get(k).getAsJsonObject().get("length").getAsInt();
				results.add(new LintResult(desc + "\n[" + ruleId + "]", filePath, Severity.ERROR, start, length));
			}
		}
		return results;
	}
